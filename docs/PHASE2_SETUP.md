...
2. Pon una ruta real de descargas, p. ej. `VITE_DEFAULT_DOWNLOAD_PATH=%USERPROFILE%\Music\MEmu Music`.
3. Opcional: `CLIP_HARBOUR_COOKIES=%USERPROFILE%\cookies_youtube\cookies_chrome.txt` (o `cookies_edge.txt`) como fallback de proceso; la sidebar también guarda la ruta en `localStorage`.

## 2. Cookies YouTube — Método A (oficial Fase 2)

La UI de la sidebar ofrece **Elegir cookies.txt** (en la cabecera de la sección) y **Actualizar cookies** (extrae del navegador bajo demanda). No uses el selector “Desde el navegador” (oculto).
...
6. Guarda el archivo **fuera del repo** y fuera de carpetas sincronizadas (Proton Drive, OneDrive…), p. ej.  
   `%USERPROFILE%\cookies_youtube\cookies_chrome.txt` o `cookies_edge.txt`.
...
.\scripts\filter-youtube-cookies.ps1 -InputPath @(
  "$env:USERPROFILE\cookies_youtube\cookies_edge.bak.txt",
  "$env:USERPROFILE\cookies_youtube\cookies_chrome.bak.txt"
) -OutputPath "$env:USERPROFILE\cookies_youtube\cookies_merged.txt"
```

9. En Clip Harbour: sidebar → **Elegir cookies.txt** → `%USERPROFILE%\cookies_youtube\cookies_merged.txt`.
...
>
> **Caducadas eliminadas (desde este fork):** cualquier archivo manual que pase a yt-dlp también se limpia de **cookies caducadas** (`expiry > 0` anterior a ahora) vía `prepare_cookie_file` en Rust, escribiendo `<nombre>.clean.txt` solo si hubo limpieza (el original no se toca). Conserva las de sesión (`expiry 0`).
>
> **Auto-refresh desde navegador (desde este fork):** al iniciar la app, Clip Harbour extrae automáticamente las cookies de sesión probando los navegadores en orden (**Firefox**, luego **Chrome**, luego **Edge**) con `yt-dlp --cookies-from-browser <browser>`, usando la primera que tenga sesión de YouTube. Las filtra/enriquece en Rust (solo dominios YouTube/Google, dedupe conservando la expiración más reciente, UTF-8 sin BOM, dominios con `.` marcados `TRUE`) y las escribe en `cookies_merged.txt`, que queda seleccionado automáticamente. Los archivos generados viven en `%APPDATA%\com.clip-harbour.app\cookies\` (dentro de la app); la carpeta legada `C:\Users\<usuario>\cookies_youtube` se sigue usando mientras contenga `.txt`. Esto evita el `403: revisa cookies de YouTube en el panel lateral` por cookies caducas. Si ninguno tiene sesión (o falla la extracción), la app conserva el archivo previo y muestra un aviso en la sidebar.
>
